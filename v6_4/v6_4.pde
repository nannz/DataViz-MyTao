...
void drawData() {
  ...
  for ( City city : coords) {
    ...
    priceLength = map(city.price, 7, 1116, 200, 290);
    ...
    if (city.year == 2015 && drawMode == 1) {  //green--2015
      ...
      vertex(width/2 + rr*cos(-PI/2+day_arc- PI*4/365), height/2 + rr*sin(-PI/2+day_arc- PI*4/365));//the two points beside it
      vertex(width/2 + rr*cos(-PI/2+day_arc+ PI*4/365), height/2 + rr*sin(-PI/2+day_arc+ PI*4/365));
      endShape(CLOSE);
